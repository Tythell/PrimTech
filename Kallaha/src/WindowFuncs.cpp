...
bool Hole(const size_t index, const int numBeans, const ImVec2 position, const ImVec2 size)
{
	ImVec2 currentCursorPos = ImGui::GetCursorPos();
	std::string label = std::to_string(numBeans) + "\n\n    " + std::to_string(index);
	ImGui::SetCursorPos(position);
	ImGui::BeginDisabled(index == 6 || index == 13);
	ImGui::SetWindowFontScale(2.f);
	bool returnResult = ImGui::Button(label.c_str(), size);
	ImGui::EndDisabled();
	ImGui::SetCursorPos(currentCursorPos);
	return returnResult;
}

void Gui_Mancala(void* ptr, bool* show)
{
	ImGui::Begin("Shit Mancala board", show, 0);

	float buttonSize = 100.f;
	float buttonOffset = 10.f;

	float xPos = ImGui::GetCursorPosX() + buttonSize + buttonOffset;
	float yPos = ImGui::GetCursorPosY() + buttonSize + buttonOffset;

	static int beansAmount[14]{
		4,4,4,4,4,4,0,
		4,4,4,4,4,4,0 };
	...
	const uint holeAmount = 14;
	const uint pocketIndex[2] = { 6,13 };

	uint holeScore[2] = { 0,0 };

	static char winner = -1;

	for (uint i = 0; i < pocketIndex[0]; i++)
	{
		holeScore[0] += beansAmount[i];
		holeScore[1] += beansAmount[i + 7];
	}

	if (winner == -1 && (!holeScore[0] || !holeScore[1]))
	{
		beansAmount[pocketIndex[0]] += holeScore[0];
		beansAmount[pocketIndex[1]] += holeScore[1];
		memset(&beansAmount[0], 0, sizeof(uint) * pocketIndex[0]);
		memset(&beansAmount[pocketIndex[0]+1], 0, sizeof(uint) * pocketIndex[0]);

		winner = (bool)(beansAmount[pocketIndex[0]] < beansAmount[pocketIndex[1]]);

		msgStr = "Player " + std::to_string(winner) + " wins";
	}

	for (uint i = 0; i < holeAmount; i++)
	{
		uint originalAmount = beansAmount[i];
		if (i == pocketIndex[0])
		{
			buttonOffset *= -1;
			buttonSize *= -1;
			yPos += buttonSize + buttonOffset;
		}
		int x = (int)(i == pocketIndex[0] || i == pocketIndex[1]);
		int ySize = std::abs(buttonSize) * (1.f + x) + (10 * x);
		if (Hole(i, beansAmount[i], ImVec2(xPos, yPos), ImVec2(std::abs(buttonSize), ySize)))
		{
			char playerIndex = i > pocketIndex[0];

			int skipper = 1;
			uint lastIndex = 0;
			uint startIndex = i;

			for (uint j = i + 1; j < i + beansAmount[i] + skipper; j++)
			{
				uint beanIndex = j % holeAmount;
				if ((
					beanIndex == pocketIndex[0] && playerIndex == 1) ||
					beanIndex == pocketIndex[1] && playerIndex == 0)
				{
					skipper++;
					continue;
				}
				lastIndex = beanIndex;
				beansAmount[beanIndex]++;
			}
			beansAmount[i] -= originalAmount;
			
			// debug oposite side
			
			uint oppositeIndex = (lastIndex + 12 - 2 * lastIndex) % 14;

			bool landOnOwnRow = false; 

			for (uint j = 0; j < 2; j++)
			{
				landOnOwnRow = landOnOwnRow ||
				ptm::inRange<uint>(startIndex, 0 + (pocketIndex[0] +1 ) *j, pocketIndex[j] - 1) &&
				ptm::inRange<uint>(lastIndex, 0 + (pocketIndex[0] + 1) * j, pocketIndex[j] - 1);
			}

			if (landOnOwnRow && beansAmount[lastIndex] == 1 && beansAmount[oppositeIndex] != 0)
			{
				uint pocket = pocketIndex[playerIndex];
				beansAmount[pocket]++;
				beansAmount[pocket] += beansAmount[oppositeIndex];

				beansAmount[lastIndex] = 0;
				beansAmount[oppositeIndex] = 0;
			}
		}

		xPos += buttonOffset + buttonSize;
	}
	ImGui::SetCursorPosY(250.f);
	ImGui::Text(msgStr.c_str());


	//std::string holeScores = "holescores: " + std::to_string(holeScore[0]) + " " + std::to_string(holeScore[1]);
	//ImGui::Text(holeScores.c_str());

	ImGui::End();

	//uint aa = 0;

	//std::function<void(void*, bool*)> foo = [&](void* ptr, bool* show)
	//{
	//	aa = 69;
	...
	if (p->selected != -1)
	{
		...
		if (pEnt->HasComponentType(PrimtTech::ec_cam))
		{
			if (ImGui::TreeNode("Camera"))
			{
				...
				float scale = mr->GetScaleOffset().x;
				...
				if (ImGui::DragFloat("Scale##cam", reinterpret_cast<float*>(&scale), 0.02f))
