...
Game::Game(int2 windowRes, HINSTANCE hInstance)
{
	//PrimtTech::ResourceHandler::ReserveMeshMemory(15);
	//PrimtTech::ResourceHandler::ReserveMaterialMemory(8);

	m_primtech.Init(L"Game", hInstance, L"wndc", windowRes.x, windowRes.y);
	...
	m_entlist.console.m_showWin.emplace_back(false, "assetlist");
	...
	m_entlist.console.m_showWin.emplace_back(false, "compView");
	m_primtech.CreateImGuiWindow(Gui_CompView, NULL, &m_entlist.console.m_showWin[u++].first);

	//m_entlist.console.m_showWin.emplace_back(true, "mancala");
	//m_primtech.CreateImGuiWindow(Gui_Mancala, NULL, &m_entlist.console.m_showWin[u++].first);
	...
	pt::Entity& playerEnt = pt::Entity::Create("Player");
	playerEnt.AddComponent<pt::MeshRef>();
	playerEnt.SetPosition(0.f, 2.f, 0.f);
	pt::PhysicsBody* pPhysBod = playerEnt.AddComponent<pt::PhysicsBody>();
	pPhysBod->CreateRigidBody(playerEnt.Transform());
	pPhysBod->SetType(rp::BodyType::KINEMATIC);
	pPhysBod->AddCapsuleColider(.5f, 1.5f);

	pt::Camera* playerCam = playerEnt.AddComponent<pt::Camera>();
	playerCam->SetPerspective(40.f, (float)windowRes.x, (float)windowRes.y, 10, 100.f);
	playerCam->SetPositionOffset(0.f, 0.f, -30.f);
	playerCam->SetRotationOffset(0.3f, 0.f, 0.f);

	pt::Entity& ent1 = pt::Entity::Create("Plane");
	ent1.SetPosition(0.f, -0.2f, 0.f);
	ent1.SetScale(21.f, 1.f, 35.f);
	ent1.AddComponent<pt::MeshRef>()->SetMesh("scaledplane.obj");
	pPhysBod = ent1.AddComponent<pt::PhysicsBody>();
	pPhysBod->CreateRigidBody(ent1.Transform());
	pPhysBod->AddBoxColider({ 11.f, 0.3f, 18.f });

	pt::Entity& kubb1 = pt::Entity::Create("kubb1");
	kubb1.SetPosition(0.f, -0.2f, 0.f);
	kubb1.SetScale(1.f, 1.f, 1.f);
	//ent1.AddComponent<pt::MeshRef>()->SetMesh("scaledplane.obj");
	//pPhysBod = ent1.AddComponent<pt::PhysicsBody>();
	//pPhysBod->CreateRigidBody(ent1.Transform());
	//pPhysBod->AddBoxColider({ 11.f, 0.3f, 18.f });
...
void Game::execCommand(std::string cmd)
{
	std::stringstream ss;
	ss.clear();
...
void Game::UpdateInput(float deltatime)
{
	...
	pt::TransformComp* pDevTransform = &PrimtTech::ComponentHandler::GetComponentByIndex<pt::TransformComp>(m_activeCamEntIndex);
	pt::Camera* pDevCam = &PrimtTech::ComponentHandler::GetComponentByIndex<pt::Camera>(1);
...
{
	pt::Entity& playerEnt = pt::Entity::GetEntity(2);
	
	pt::PhysicsBody* physBod = playerEnt.GetComponent<pt::PhysicsBody>();
	//physBod-
}
...
void Game::Run()
{
	...
	while (m_primtech.IsOpen())
	{
		...
		{
			Game::Messages postMsg = m_msgQueue.front();
			switch (postMsg)
			{
			case Game::Messages::eQuit:
				m_primtech.Close();
				break;
			case Game::Messages::eToggleMouse:
				m_primtech.ToggleMouse();
				break;
			case Game::Messages::eHideMouse:
				m_primtech.HideCursor();
				break;
			case Game::Messages::eShowMouse:
