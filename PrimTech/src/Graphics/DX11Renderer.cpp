...
namespace PrimtTech
{
		...
		m_shadowmap(1024 * shadowQuality, 1024 * shadowQuality), m_viewport(0.f, 0.f, m_winDimensions->x, m_winDimensions->y)
	{
		...
		m_guiHandler.ImGuiInit(*m_pHWND, device, dc);
	...
	bool Renderer::InitRastNSampState()
	{
		...
		D3D11_TEXTURE_ADDRESS_MODE adressMode = 
			D3D11_TEXTURE_ADDRESS_BORDER;
			//D3D11_TEXTURE_ADDRESS_WRAP;

		sampDesc.AddressU = adressMode;
		sampDesc.AddressV = adressMode;
		sampDesc.AddressW = adressMode;
	...
	void drawMeshes(std::vector<MeshRef>& rMeshrefs, std::vector<TransformComp>& rTransforms, std::vector<MeshPrefabRef>& rPrefabs,
		Buffer<hlsl::cbpWorldTransforms3D>& transformBuffer, Buffer<hlsl::cbpMaterialBuffer>* pMAtBuffer, ID3D11DeviceContext*& dc, float deltatime, uint& drawCalls)
	{
	...
	void Renderer::Render(const float& deltatime)
	{
		...
		m_lightbuffer.Data().cbShadowBias = 0.5f;
		...
		m_transformBuffer.MapBuffer();
		drawMeshes(rMeshrefs, rTransforms, rPrefabrefs, m_transformBuffer, &m_materialBuffer, dc, deltatime, m_drawCalls);
